...
function Sidebar({ page, setPage, user }) {
  ...
  return (
    <div className="sidebar">
      ...
      {user?.role === 'Professor' ? (
        <>
          <h3 style={{ color: `${page === 0 ? 'white' : 'gray'}` }} onClick={() => setPage(0)}>
            Home
          </h3>
          <h3 style={{ color: `${page === 1 ? 'white' : 'gray'}` }} onClick={() => setPage(1)}>
            Create Tutorials
          </h3>
          <h3 style={{ color: `${page === 2 ? 'white' : 'gray'}` }} onClick={() => setPage(2)}>
            Evaluate Reports
          </h3>
          <h3 style={{ color: `${page === 3 ? 'white' : 'gray'}` }} onClick={() => setPage(3)}>
            Students
          </h3>
        </>
      ) : (
