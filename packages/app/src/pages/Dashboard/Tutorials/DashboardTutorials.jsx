...
function DashboardTutorials({ user }) {
  ...
  const removeTutorial = useCallback(
    async (tutorial) => {
      const tutorialArray = _remove(
        [...savedTutorials],
        (item) => !_isEqual(item.tutorial, tutorial),
      ); // eslint-disable-line
          ...
          _map(savedTutorials, ({ tutorial }, index) => (
            <div key={index} className="tutorial__item">
              <div
                ...
                <Chip
                  label={tutorial?.level[0].toUpperCase() + tutorial?.level.slice(1)}
                  size="small"
                  style={{
        ...
        {completedTutorials.length > 0 ? (
          _map(completedTutorials, ({ tutorial, time }, index) => (
            <div key={index} className="tutorial__item">
              ...
              />
              <p>
                <span style={{ color: 'lightgreen' }}>Completed on</span> : {Date(time)}
              </p>
            </div>
          ))
        ) : (
