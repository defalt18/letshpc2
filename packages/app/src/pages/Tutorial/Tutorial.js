...
function Tutorial(props) {
  ...
  const { code, theory, testcases, title, level } = tutorialDetails || Tutorial_Default;
  ...
  const markComplete = useCallback(async () => {
    ...
    const updatedUser = {
      ...user,
      completedTutorials: [...tutorialList, { tutorial: tutorialDetails, time: Date.now() }],
    };
  ...
  return (
    <div className="tutorials__page">
      <div className="tut__head">
        <IconButton aria-label="back" style={{ color: 'white' }} onClick={() => history.goBack()}>
          <ArrowBackIcon />
        </IconButton>
        <h2>{title}</h2>
        <Chip
      ...
      <div className="content__window">
          ...
          <div className="samp">
            ...
            <ButtonGroup style={{ position: 'absolute', right: '10px' }}>
              ...
              <Button
                ...
                <SshModal OS="mac" background="black" textcolor="white" />
              </Button>
            </ButtonGroup>
          </div>
        ...
        <div className="codeboxes" id="output" style={{ display: 'none', margin: '20px 0' }}>
